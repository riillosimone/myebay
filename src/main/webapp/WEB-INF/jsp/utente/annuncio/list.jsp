			                ...
			                <tbody>
			                	<c:forEach items="${annuncio_list_attr }" var="annuncioItem">
									<tr>
										...
										<td>
										<c:choose>
												<c:when test="${!isAutenticato || annuncioItem.aperto == false}"><a class="btn btn-sm btn-outline-secondary" href="${pageContext.request.contextPath}/annuncio/show/${annuncioItem.id }">Visualizza</a>
												<div style="float: right;">Venduto</div>
												</c:when>
												<c:otherwise>
